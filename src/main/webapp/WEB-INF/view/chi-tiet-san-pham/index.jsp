...
<a style="margin: 20px" href="/chi-tiet-san-pham/view-add" type="button"
...
<table class="table table-hover">
    ...
    <c:forEach items="${danhSach.content}" var="cv" varStatus="stt">
        <tr>
            ...
            <td>${cv.idSP.ma}</td>
            <td>${cv.idSP.ten}</td>
            <td>${cv.idMauSac.ten}</td>
            <td>${cv.idDongSP.ten}</td>
            <td>${cv.idNsx.ten}</td>
            <td>${cv.namBH}</td>
            <td>${cv.soLuongTon}</td>
            <td>${cv.giaNhap}</td>
            ...
            <td>${cv.moTa}</td>
            <td>
                <a href="/chi-tiet-san-pham/view-update/${cv.id}"
                   class="btn btn-warning btn-sm"><i class="fa-solid fa-pen-to-square"></i></a>
                <a href="/chi-tiet-san-pham/remove/${cv.id}"
...
<nav aria-label="Page navigation example">
    <ul class="pagination justify-content-center">
        <li class="page-item">
            <a class="page-link" href="/chi-tiet-san-pham/hien-thi?page=1">Trước</a>
        </li>
        <c:forEach begin="0" end="${danhSach.totalPages -1}" varStatus="loop">
            <li class="page-item">
                <a class="page-link"
                   href="/chi-tiet-san-pham/hien-thi?page=${loop.index + 1}">${loop.index + 1}</a>
            </li>
        </c:forEach>
        <li class="page-item">
            <a class="page-link" href="/chi-tiet-san-pham/hien-thi?page=${danhSach.totalPages}">Sau</a>
        </li>
    </ul>
</nav>
